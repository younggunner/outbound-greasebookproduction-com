/* gb-attr.js — cross-domain attribution + page-journey breadcrumb.
   Goal: replace what Cometly used to capture (referrer chain + on-site path).
   Mechanism: first-party cookie gb_attr on .greasebook.com (read by quiz subdomain)
              + gb_journey in localStorage (FIFO 20). Origin referrer never overwritten
              once set (90-day TTL).
   Read by:   instapage-embed-snippet.html on quiz.greasebook.com → Typeform hidden
              fields origin_referrer + page_journey → typeform-ga4-relay v15
...
(function () {
  try {
    ...
    var sameSite = refHost === location.hostname || refHost.indexOf(DOMAIN.replace(/^\./,'')) >= 0;

    // Decode existing cookie (or seed fresh).
    var prev = {};
    var raw = readCookie(COOKIE);
    if (raw) { try { prev = JSON.parse(raw); } catch (e) { prev = {}; } }

    // Origin referrer: set once, never overwrite. Only stamp on external entry.
    if (!prev.origin_referrer && ref && !sameSite) {
      prev.origin_referrer = ref;
      prev.origin_at = now;
    }

    // Capture UTMs from the URL (any page, not just landing) — first-touch wins.
    ...
    // Append current page to journey (localStorage = full, cookie = compact tail).
    var journey = [];
    try {
      var j = localStorage.getItem('gb_journey');
    ...
    if (!last || last.u !== path) {
      ...
      try { localStorage.setItem('gb_journey', JSON.stringify(journey)); } catch (e) {}
    }

    // Compact cookie payload (tail only, to stay under 4KB cookie limit).
